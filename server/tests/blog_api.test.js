...
describe('POST and DELETE blogs', () => {
  ...
  beforeEach(async () => {
    ...
    const loggedInUser = await api.post('/api/login').send(login).expect(200)
  ...
  test('http post without likes has 0 likes', async () => {
    ...
    const blogPost = Object.values(blogPostJson).find(
...
describe('PUT: a blog is updated', () => {
  test('a blog can be updated', async () => {
    ...
    const blogsAtEnd = await helper.blogsInDb()
    const blogPost = Object.values(blogsAtEnd).find(
